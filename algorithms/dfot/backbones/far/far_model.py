...
class FAR_AttnProcessor2_0:
    ...
    def __call__(
        ...
        if image_rotary_emb is not None:
            ...
            # print('query_rotary_emb', query_rotary_emb[0].shape)
            # exit(0)
...
class FARDiT(BaseBackbone):
    ...
    def __init__(
        ...
        axes_dims_rope = cfg.get('axes_dims_rope', (16, 24, 24))
        self.slope_scale = cfg.get('slope_scale', 0.0)
        channels, resolution, *_ = x_shape
        self.patch_size = cfg.patch_size
        self.out_channels = self.patch_size**2 * channels
        self.inner_dim = cfg.hidden_size

        self.x_embedder = torch.nn.Linear(channels * self.patch_size * self.patch_size, self.inner_dim)
        ...
        # print('hidden_size', self.inner_dim, 'patch_size', self.patch_size, 'axes_dims_rope', axes_dims_rope
            #   , 'slope_scale', self.slope_scale, 'num_heads', cfg.num_heads, 'depth', cfg.depth, 'attention_head_dim', self.inner_dim // cfg.num_heads)
        self.transformer_blocks = nn.ModuleList([
            FAR_TransformerBlock(
                dim=self.inner_dim,
                num_attention_heads=cfg.num_heads,
                attention_head_dim=self.inner_dim // cfg.num_heads,
    ...
    def initialize_weights(self):
        ...
        if hasattr(self, 'external_cond_embedding'):
            nn.init.normal_(self.external_cond_embedding.embedding_table.weight, std=0.02)
    ...
    def _build_causal_mask(self, input_shape, device, dtype):
        ...
        linear_bias = self.slope_scale * get_relative_positions(seq_len)
    ...
    def forward(
        self,
        x: torch.Tensor,
        noise_levels: torch.LongTensor = None,
        external_cond: torch.LongTensor = None,
        ...
        token_per_frame = (height // self.patch_size) * (width // self.patch_size)
        # print('hidden_states', x.shape, x.dtype)
        # print('noise_levels', noise_levels.shape, noise_levels.dtype)
        # step 1: pack latent sequence (and compute rope)
        hidden_states = self._pack_latent_sequence(x, patch_size=self.patch_size)
        ...
        if noise_levels.dim() == 1:
            noise_levels = noise_levels.unsqueeze(-1).repeat((1, num_frames))
        ...
        timestep = rearrange(noise_levels, 'b t -> (b t)')
        ...
        if self.external_cond_embedding is not None:
            if self.external_cond_type == 'label':
                # print("conditions['label']", external_cond.shape, '\n\n')
                label_emb = self.external_cond_embedding(external_cond[:, 0]).unsqueeze(1)
                temb = temb + label_emb
            elif self.external_cond_type == 'action':
                cond = rearrange(torch.argmax(external_cond, dim=-1), 'b t -> (b t)')
                action_emb = self.external_cond_embedding(cond)
...
if __name__ == "__main__":
    ...
    model = FARDiT(cfg, x_shape=torch.Size([32, 8, 8]), max_tokens=1024, external_cond_type='label', external_cond_num_classes=101, external_cond_dim=1024)
    print(model)
    x = torch.randn(4, 16, 32, 8, 8)  # batch_size=2, num_frames=16, channels=3, height=32, width=32
    noise_levels = torch.randint(0, 1000, (4, 16))
    external_cond = torch.randint(0, 101, (4,))
    output = model(x, noise_levels=noise_levels, external_cond=external_cond)
    print(output.sample.shape)  # should be (2, 16, 3,
