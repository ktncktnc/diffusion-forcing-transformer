...
class DiTBlock(nn.Module):
    ...
    def forward(
        ...
        if self.use_mlp:
            ...
            x = x + gate_mlp * self.mlp(torch.cat([x, reference], dim=-1))
        return x
...
class ResNetBlock(nn.Module):
    ...
    def forward(
        ...
        x, gate_mlp1 = self.norm1(x, c)
        x = x + gate_mlp1 * self.mlp1(x)
        if self.use_mlp:
            x, gate_mlp2 = self.norm2(x, c)
            x = x + gate_mlp2 * self.mlp2(x)
        return x